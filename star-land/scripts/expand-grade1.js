...
// ========== 题目数据定义 ==========
// 格式：[年级][单元号][课时号] = { extra: [4道追加题], l4: {...}, l5: {...} }
    ...
    1: { // Hello!
      1: { // 打招呼
        extra: [
          ...
          { id: 'e1u1l1q5', type: 'fill', question: 'Hi和Hello意思相同，但Hi更____。', answer: 'casual', hint: '哪个更简短？', explanation: 'Hi比Hello更简短随意，朋友之间常用' },
    ...
    5: { // Family
      ...
      l4: {
        ...
        questions: [
          ...
          { id: 'e1u5l4q4', type: 'drag', question: '把关系拖到正确位置：爸爸的姐妹叫____', answer: 'Aunt', hint: '姑姑用英语怎么说？', explanation: '爸爸的姐妹是Aunt姑姑' },
...
const unitTitles = {
  1: {
    1: { title: 'Hello!', reviewTitle: 'Hello! Review', lifeTitle: 'Hello! in Life' },
    2: { title: 'Colors', reviewTitle: 'Colors Review', lifeTitle: 'Colors in Life' },
    3: { title: 'Numbers', reviewTitle: 'Numbers Review', lifeTitle: 'Numbers in Life' },
    4: { title: 'Animals', reviewTitle: 'Animals Review', lifeTitle: 'Animals in Life' },
    5: { title: 'Family', reviewTitle: 'Family Review', lifeTitle: 'Family in Life' },
    6: { title: 'Food', reviewTitle: 'Food Review', lifeTitle: 'Food in Life' },
    7: { title: 'Body', reviewTitle: 'Body Review', lifeTitle: 'Body in Life' },
    8: { title: 'Classroom', reviewTitle: 'Classroom Review', lifeTitle: 'Classroom in Life' },
  }
};

// 生成第4课和第5课
function generateLesson4(grade, unitNum) {
  const data = questionData[grade]?.[unitNum]?.l4;
  if (!data) return null;
  const prefix = `e${grade}u${unitNum}`;
  return {
    id: `${prefix}l4`,
    title: `${unitTitles[grade][unitNum].reviewTitle}`,
    order: 4,
...
function generateLesson5(grade, unitNum) {
  const data = questionData[grade]?.[unitNum]?.l5;
  if (!data) return null;
  const prefix = `e${grade}u${unitNum}`;
  return {
    id: `${prefix}l5`,
    title: `${unitTitles[grade][unitNum].lifeTitle}`,
    order: 5,
...
// 主处理函数
function processGrade1() {
  const filePath = path.join(__dirname, '..', 'src', 'data', 'english', 'grade1.ts');
  let content = fs.readFileSync(filePath, 'utf-8');

  // 为每个单元的每个现有课时追加4道题
  for (let unitNum = 1; unitNum <= 8; unitNum++) {
    for (let lessonNum = 1; lessonNum <= 3; lessonNum++) {
      const extra = questionData[1]?.[unitNum]?.[lessonNum]?.extra;
      if (!extra) continue;

      // 找到该课时的practiceQuestions数组末尾
      const lessonId = `e1u${unitNum}l${lessonNum}`;
      // 找到该课时最后一道题的结束位置
      const lastQuestionId = `e1u${unitNum}l${lessonNum}q3`;
      ...
      if (match) {
        ...
        const extraQuestionsStr = extra.map(q => {
          let qStr = `,\n          {\n            id: '${q.id}',\n            type: '${q.type}',\n            question: '${q.question}',\n`;
          if (q.options) {
            qStr += `            options: [${q.options.map(o => `'${o}'`).join(', ')}],\n`;
          }
    ...
    const unitId = `e1u${unitNum}`;
    // 找到该单元最后一个lesson的结束位置
    // 查找 lessons 数组中最后一个 } 后的位置
    const unitPattern = new RegExp(`id: '${unitId}'[\\s\\S]*?lessons: \\[`, 'g');
    ...
    if (unitMatch) {
      ...
      if (unitStartIdx !== -1) {
        // 从unitStartIdx开始，找到匹配的 lessons: [ ... ] 结束
        let braceCount = 0;
        ...
        if (lessonsEndIdx !== -1) {
          const l4 = generateLesson4(1, unitNum);
          const l5 = generateLesson5(1, unitNum);
...
function serializeLesson(lesson) {
  ...
  for (const block of lesson.content) {
    str += `          { type: '${block.type}', content: \`${block.content}\`, animationType: '${block.animationType}'`;
